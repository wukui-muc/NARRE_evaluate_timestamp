...
def numerize(data):
    ...
    timeID.sort()
    print('timeID:%d',timeID)
    time=np.array(time)
    i=1
    for line in timeID:
        id=np.where(time==line)[0]
        time[id]=i
        i=i+1
    new_time=list(time)
    data['reviewTime']=new_time
...
if __name__ == '__main__':
    ...
    print("P_REVIEW:",P_REVIEW)
    print("MAV_DF:",MAX_DF)
    print("MAXVOCAB:",MAX_VOCAB)
    ...
    if yelp_data:
        ...
        for line in file:
            ...
            timestamp.append(str(js['reviewTime']))

    data_frame = {'user_id': pd.Series(users_id), 'item_id': pd.Series(items_id),
                  'ratings': pd.Series(ratings), 'reviews': pd.Series(reviews),'reviewTime':pd.Series(timestamp)}
    data = pd.DataFrame(data_frame)     # [['user_id', 'item_id', 'ratings', 'reviews']]
    data = data[['user_id','item_id','ratings','reviews','reviewTime']]

    del users_id, items_id, ratings, reviews,timestamp
    ...
    timeNum_all=get_count(data,'reviewTime')
    time_size=max(timeNum_all)
    ...
    for i in data_train.values:
        ...
        # 收集所有用户的评论，一个用户ID在user_review_dict占据一行元素
        # user_reviews_dict中的每一条评论的timestamp对应在user_reviews_time_dict中
        if i[0] in user_reviews_dict:
            user_reviews_dict[i[0]].append(str_review)
            user_iid_dict[i[0]].append(i[1])
            user_reviews_time_dict[i[0]].append(i[4])
        else:
            user_reviews_dict[i[0]] = [str_review]
            user_iid_dict[i[0]] = [i[1]]
            user_reviews_time_dict[i[0]]=[i[4]]

        # 收集所有item的评论，一个itemID在item_reviews_dict占据一行元素
        # item_reviews_dict中的每条评论的时间戳对应在item_reviews_time_dict的相同位置
        if i[1] in item_reviews_dict:
            item_reviews_dict[i[1]].append(str_review)
            item_uid_dict[i[1]].append(i[0])
            item_reviews_time_dict[i[1]].append(i[4])
        else:
            item_reviews_dict[i[1]] = [str_review]
            item_uid_dict[i[1]] = [i[0]]
            item_reviews_time_dict[i[1]]=[i[4]]
    ...
    def padding_time(time,num, pad_time):
        if len(time) >= num:
            new_time=time[:num]
        else:
            new_time=time+ [pad_time]*(num-len(time))
        return new_time
    ...
    for i in range(userNum):
        ...
        u_reviewList = []
        u_review_time=user_reviews_time_dict[i]
        ...
        userreview_timelist.append(padding_time(u_review_time,u_pReviewLen,0)) #对时间戳做与ID相同补齐操作
    ...
    for i in range(itemNum):
        ...
        i_review_time=item_reviews_time_dict[i]
        ...
        itemreview_timelist.append(padding_time(i_review_time,i_pReviewLen,0))
    ...
    np.save(f"{save_folder}/train/userreview_timelist.npy",userreview_timelist)
